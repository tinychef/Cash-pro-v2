...
import { currency, shortDate, totalReceivables, type Client } from "@cash-pro/core";
import {
  useCustomers,
  useInvoices,
  usePayments,
...
import { Search, Plus, Users, Phone, Mail, Pencil, Trash2, AlertCircle } from "lucide-react";
import { LoadingState, ErrorState, EmptyState } from "@/components/states";

const emptyClient = { name: "", email: "", phone: "", address: "", notes: "" };

export default function ClientsPage() {
  const { data: clients = [], isLoading, error } = useCustomers();
  ...
  const handleSave = async () => {
    if (!form.name) return;
    try {
      if (editId) await updateCustomer.mutateAsync({ id: editId, ...form });
      else await createCustomer.mutateAsync(form);
      setDialogOpen(false);
    } catch {
      /* toast handled */
    }
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 max-w-7xl mx-auto">
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">{currency(receivables)}</p>
            </div>
      ...
      <div className="relative">
        ...
        <Input placeholder="Buscar clientes..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 rounded-xl border-0 bg-secondary" />
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Users} title="No se encontraron clientes" description="Agrega tu primer cliente para empezar a facturar." />
      ) : (
        <div className="space-y-3">
          {filtered.map((c) => {
            const balance = clientBalance(c.id);
            const count = clientInvoiceCount(c.id);
            const hasOverdue = invoices.some((i) => i.clientId === c.id && i.status === "overdue");
            return (
              <Card key={c.id} className="border-0 shadow-sm hover:shadow-md transition-shadow group">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-right mr-2">
                        {balance > 0 ? (
                          <>
                            <p className={`text-sm font-bold ${hasOverdue ? "text-red-500" : "text-amber-500"}`}>{currency(balance)}</p>
                            <Badge variant="secondary" className={`text-[10px] ${hasOverdue ? "status-overdue" : "status-partial"}`}>
                              {hasOverdue ? "Vencido" : "Pendiente"}
                            </Badge>
                          </>
                        ) : (
                          <Badge variant="secondary" className="status-paid text-[10px]">Al día</Badge>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border text-xs text-muted-foreground">
                    ...
                    <span>Desde {shortDate(c.createdAt.slice(0, 10))}</span>
                  </div>
                </CardContent>
              </Card>
            );
