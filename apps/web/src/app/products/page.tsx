...
import { currency, pct, marginRatio, type Product } from "@cash-pro/core";
import {
  useProducts,
  useCreateProduct,
  useUpdateProduct,
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5 max-w-7xl mx-auto">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Package} title="No se encontraron productos" description="Crea tu primer producto con su costo y precio de venta." />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {filtered.map((p) => {
            const m = marginRatio(p.salePrice, p.purchasePrice);
            const lowStock = p.stock < p.minStock;
            return (
              <Card key={p.id} className="border-0 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
                <CardContent className="p-4">
                  ...
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-bold ${lowStock ? "text-red-500" : "text-foreground"}`}>{p.stock}</span>
                      {lowStock && (
                    ...
                    <span className="text-[11px] text-muted-foreground">Mín: {p.minStock}</span>
                  </div>
                </CardContent>
              </Card>
            );
            ...
            {form.salePrice > 0 && (
              <div className="rounded-lg bg-secondary p-3 flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-primary">
                  {pct(marginRatio(form.salePrice, form.purchasePrice))} · {currency(form.salePrice - form.purchasePrice)}/u
                </span>
              </div>
            )}
